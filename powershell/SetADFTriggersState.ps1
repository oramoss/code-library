...
-- Jeff Moss    02-JUN-2020    Fixed error where Name switch to various calls had extra space.
-- Jeff Moss    07-JUN-2020    Changed C:\Data to $(System.DefaultWorkingDirectory)
-- Jeff Moss    07-JUN-2020    Changed $(System.DefaultWorkingDirectory) to parameter 
--                             $TriggerFileLocation
-- -----------------------------------------------------------------------------
#>
...
Write-Output " TriggerFileLocation:$TriggerFileLocation"
...
if (!$df)
{
  ...
  If ($State -eq "StartPriorEnabled") {
    if (Test-Path -Path $TriggerFileLocation\trigger_state_$ReleaseIdentifier.txt) {
      Write-Output "Prior Triggers Enabled File $TriggerFileLocation\trigger_state_$ReleaseIdentifier.txt exists..."
      $triggersADF = Get-Content -Path $TriggerFileLocation\trigger_state_$ReleaseIdentifier.txt
    }
    else {
      Write-Output "Prior Triggers Enabled File $TriggerFileLocation\trigger_state_$ReleaseIdentifier.txt does not exist..."
    }
  }
  else {
  ...
  #=======================================================================================
   Write-Output "Contents of $TriggerFileLocation\trigger_state_$ReleaseIdentifier.txt..."
   $triggersADF
  ...
  elseif ($State -eq "Stop") {
    ...
    $triggersADF | ForEach-Object { 
      if($_.RuntimeState -eq "Started") {
        ...
        Add-Content -Path $TriggerFileLocation\trigger_state_$ReleaseIdentifier.txt -Value $_.Name
      }
      else {
