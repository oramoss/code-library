# Azure DevOps
Links related to Azure DevOps.

## Pipelines
- [Setting up a CI/CD pipeline for Azure Functions](https://www.azuredevopslabs.com/labs/vstsextend/azurefunctions/)
- [Azure DevOps: store the TAG version in a system variable](https://medium.com/@lucanardelli/azure-devops-store-the-tag-version-in-a-system-variable-3ad5c8a26d93)
- [Azure DevOps - CI CD - Build And Release Pipeline - Azure Function](https://www.c-sharpcorner.com/article/azure-devops-ci-cd-build-and-release-pipeline-azure-function)

## Deploying
- [Azure Functions: Deploying from VS Code and Azure DevOps](https://cloudskills.io/blog/azure-functions-deploy)
- [Auto-generating release notes in Azure DevOps pipelines](https://willys-cave.ghost.io/auto-generating-release-notes-in-azure-devops-pipelines/)
- [Release Triggers](https://docs.microsoft.com/en-us/azure/devops/pipelines/release/triggers?view=azure-devops)

## DevOps API
- [Getting started with Azure DevOps API with PowerShell](https://dev.to/omiossec/getting-started-with-azure-devops-api-with-powershell-59nn)
- [API Documentation](https://github.com/MethodsAndPractices/vsteam/tree/master/.docs)

## Extensions
- [GitVersion](https://gitversion.net/docs/build-server-support/build-server/azure-devops)
