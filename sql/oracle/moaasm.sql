...
,      count(*)        "DiskCnt"
FROM gv$asm_disk d, gv$asm_diskgroup g
WHERE d.inst_id = g.inst_id
and d.group_number = g.group_number and
...
from   GV$asm_file f,GV$asm_diskgroup g
where  f.group_number=g.group_number
group by g.inst_id,g.name,f.TYPE,f.BLOCK_SIZE,f.STRIPED
order by 1,2,3
